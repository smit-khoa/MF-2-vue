...
- Bug fixes folded in (from prior code review, all verified by reading source):
  - **Centralized 401 + network-error handling** in api layer; stop `checkAuth()` from silently treating a transient network failure as "logged out".
  - **Race in `setCurrentBusiness`**: it fires `fetchBusinessRoles` + `fetchOnboardingProgress` un-awaited and un-cancelled → fast business switch lets a stale response overwrite the fresh one. Use the existing `AbortController` support in `api`.
  - **Dead logic**: `fetchOnboardingProgress` hardcodes `is_sync_asset: false` with the real branch commented out — remove or restore, do not leave dead code.
  - **`initialize_promise` cannot retry** after a network failure (only `logout()` resets it) → make it resettable on failure.
...
Already declared in CLAUDE.md but the directory does not exist yet. This creates it.

```
.claude/features/
├── README.md      # Index table: feature | remote | route | roles | status | doc link
├── _TEMPLATE.md   # Mandatory shape
├── auth-flow.md             # real logic exists in shell → write first
├── role-feature-gating.md   # real logic exists → write first
