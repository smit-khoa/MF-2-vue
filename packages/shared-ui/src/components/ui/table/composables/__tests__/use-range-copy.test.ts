...
import { computePickerDefault, deletePreset, findSubsetPresetByColumn, getPresets, hashColumnSet, savePreset, COPY_PRESET_CAP } from "../range-copy-presets"
...
// ── hashColumnSet ──────────────────────────────────────────────
describe("hashColumnSet", () => {
    it("is order-independent", () => {
        expect(hashColumnSet(["b", "a"])).toBe(hashColumnSet(["a", "b"]))
        expect(hashColumnSet(["a", "b"])).toBe("a|b")
    })

    it("returns empty string for empty input", () => {
        expect(hashColumnSet([])).toBe("")
    })
})

// ── presets (localStorage) ─────────────────────────────────────
describe("presets localStorage round-trip", () => {
    const table = "test_table"
    beforeEach(() => localStorage.clear())

    it("saves and reads back a preset", () => {
        savePreset(table, "a|b", ["a"], ["a", "b"], true)
        const presets = getPresets(table)
        expect(presets["a|b"]).toMatchObject({ selectedKeys: ["a"], allKeys: ["a", "b"], copyHeader: true })
        expect(typeof presets["a|b"]?.savedAt).toBe("number")
    })

    it("deletes a preset", () => {
        savePreset(table, "a|b", ["a"], ["a", "b"], false)
        deletePreset(table, "a|b")
        expect(getPresets(table)["a|b"]).toBeUndefined()
    })

    it("LRU-evicts oldest when over cap", () => {
        for (let i = 0; i < COPY_PRESET_CAP + 5; i++) {
            savePreset(table, `h${i}`, [`k${i}`], [`k${i}`], false)
        }
        const presets = getPresets(table)
        expect(Object.keys(presets).length).toBeLessThanOrEqual(COPY_PRESET_CAP)
        // oldest (h0) evicted, newest kept
        expect(presets["h0"]).toBeUndefined()
        expect(presets[`h${COPY_PRESET_CAP + 4}`]).toBeDefined()
    })
})

// ── findSubsetPresetByColumn ───────────────────────────────────
describe("findSubsetPresetByColumn", () => {
    const entries = [
        { key: "fb_name", parentKey: "fb", label: "", field: "fb_name" },
        { key: "fb_id", parentKey: "fb", label: "", field: "fb_id" }
    ]

    it("covers a column when at least one sub-key is in the preset", () => {
        const presets = { h1: { selectedKeys: ["fb_name"], allKeys: ["fb_name", "fb_id"], copyHeader: false, savedAt: 1 } }
        expect(findSubsetPresetByColumn(entries, presets)?.selectedKeys).toEqual(["fb_name"])
    })

    it("returns null when no sub-key matches", () => {
        const presets = { h1: { selectedKeys: ["other"], allKeys: ["other"], copyHeader: false, savedAt: 1 } }
        expect(findSubsetPresetByColumn(entries, presets)).toBeNull()
    })

    it("picks newest when multiple match", () => {
        const presets = {
            old: { selectedKeys: ["fb_name"], allKeys: ["fb_name"], copyHeader: false, savedAt: 1 },
            recent: { selectedKeys: ["fb_id"], allKeys: ["fb_id"], copyHeader: true, savedAt: 99 }
        }
        expect(findSubsetPresetByColumn(entries, presets)?.copyHeader).toBe(true)
    })
})

// ── computePickerDefault ───────────────────────────────────────
describe("computePickerDefault", () => {
    const entries = [
        { key: "a", parentKey: "a", label: "", field: "a" },
        { key: "b", parentKey: "b", label: "", field: "b" },
        { key: "c", parentKey: "c", label: "", field: "c" }
    ]

    it("ticks ever-ticked keys, unticks ever-shown-never-ticked, ticks unseen", () => {
        const presets = {
            h: { selectedKeys: ["a"], allKeys: ["a", "b"], copyHeader: false, savedAt: 1 }
        }
        // a: everTicked → tick. b: shown but never ticked → untick. c: unseen → tick.
        expect(computePickerDefault(entries, presets)).toEqual(["a", "c"])
    })
})
